...
to setup-patches ;; procedure to assign plant species to each patch and identify neighbors
  ...
  ask patches
  [
    set habitat sum [ vegetation-volume ] of patches in-radius veg-radius
  ]
  ask patches
  [
    let radius patches in-radius veg-radius
    set x count radius
    set max-density floor ((habitat / (16 * x)) * 10)
  ]
end
...
to setup-blocks
  ...
  ask patches
  [
    set block-mates patches with [block-number = [block-number] of myself]
  ]
end
...
to setup-birds
  ...
  [
    let suitable-habitat patches in-radius 1 with [ habitat > 10 ]
...
to set-density
  ...
  [
    ...
    set habitat sum [ vegetation-volume ] of patches in-radius 1
  ]
end
