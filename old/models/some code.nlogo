...
to setup
  ...
  setup-patches
  setup-birds
  set dispersal-distance 3 ;; we should explore this variable - maybe create a slider
  reset-ticks
end
...
to setup-patches
  ...
  [
    loop
    [ set vegetation-volume round random-exponential 3 ;; didn't feel like making slider
...
to go
  calculate-bird-density ;; I currently am calculating bird density in multiple places in the code - not sure of the best way to do this
  birds-explore
  birds-reproduce
  kill-birds
  tick
...
to birds-explore
   ...
   ask turtles
  [
    if max-bird-density > 0 and bird-density <= max-bird-density
    [set settled? 1
     set color red
     stop]
    ;let my-habitat [ habitat ] of patch-here ;; don't think we need this in this version of the code
    let suitable-habitat other patches in-radius dispersal-distance with [bird-density > 0 and bird-density < max-bird-density]
    if any? suitable-habitat in-radius dispersal-distance
    [
      move-to one-of suitable-habitat
      set settled? 1
      set color yellow
      set bird-density count turtles-here ;; wondering if we should update this every time within the bird procedures?
    ]
