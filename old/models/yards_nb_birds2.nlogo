...
to setup-birds
  ...
  ask turtles
  [
    if [block-veg] of patch-here > 5 [set settled? 1 set color red]
  ]
end
...
to move-birds
  ...
  ask turtles with [settled? = 0]
  [
    let suitable-habitat patches in-radius 2 with [ block-veg > 5 ]
    ifelse any? suitable-habitat [ move-to one-of suitable-habitat set settled? 1 set color red]
    [fd 1]
  ]
end
