...
to update-estimates
  ...
  ask patches
  [
    set yard-bird-estimate round (bird-density * (vegetation-volume * ((random-float 1) / 2)))
    print word "estimate=" (yard-bird-estimate * .1)
  ]
end
...
to change-bird-love ;; adjust this procedure to increase bird love based on birds observed in yard
  ...
  [
    if change-chance < (yard-bird-estimate * .1) [ set bird-love bird-love + 1 set change-count change-count + 1 ]
  ]
