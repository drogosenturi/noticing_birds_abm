...
to setup-patches
  ...
  [
    ...
    set max-bird-density floor ((habitat / (16 * count-patches)) * 5) ;; adjust to make bird populations more sensitive to habitat values
    set bird-density 0
  ]
end
...
to birds-explore
  ...
  [
    ...
    [
      set settled? 1
      set color red
      ;stop ;;not sure why this was here but it prevents the dispersal distance variable from working
    ]
...
to add-remove-vegetation ;; add vegetation if you love birds and you don't have enough in your yard, remove if you hate birds - update habitat variable
  ...
  [
    ...
    [ if vegetation-volume < 16 [set vegetation-volume vegetation-volume + 1 ] ]
      [ if random-float 1 < (yard-bird-estimate * 0.2) [set vegetation-volume vegetation-volume + 1 ] ]
  ]
