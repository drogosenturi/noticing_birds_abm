...
to update-estimates
  ...
  ask patches
  [
    ;; need less 0's from this equation ****
...
to change-bird-love ;; adjust this procedure to increase bird love based on birds observed in yard
  ...
  ask patches
  [
    ;; normalize bird estimates btwn 0 and 1 to compare to random-float 1
    ;; but maybe imply some limit to how many times patches can change in an amount of time
    ;; *normalizing leads to many 0's, which will always lose to the random roll. How can we compare to
    ;; random # without introducing 0's?
    ...
    ;set log-est (1 / (1 + (exp (-1 * (yard-bird-estimate - mean [bird-density] of patches))))) ;; 'set' for graphing
    ;print word "log-est" log-est
    ;; consider the 25%
    if random-float .05 >= change-chance
  ...
  ;; currently commented out for testing of bird-love procedure
  ask patches
  [
