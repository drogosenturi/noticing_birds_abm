...
to sell-house ;; 3000/1,000,000 homes sold per year, 3% chance for house to sell and variables to reset
  ...
  [
    ...
    [
      set vegetation-volume random 4
      set bird-love 1
    ]
  ]
end
...
to update-estimates ;; neighbor-bird-estimate is divided by the count of neighbors to normalize the comparison of yard-bird-estimate to neighbor-bird-estimate when adding or removing vegetation
  ...
  ;; estimates are higher overall, up to 250% inaccurate
  ask patches with [vegetation-volume > 8]
  [
...
;  ]
  ...
  [
    ...
    [
      if random-float 1 > .95 [ set bird-love bird-love + 1 set change-count change-count + 1 ]
    ]
  ]
...
to output
  ...
  file-print (word count patches with [bird-love = 2] ",")
  file-close
