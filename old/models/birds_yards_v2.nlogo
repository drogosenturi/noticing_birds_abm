...
[
  ...
  adjacent-block ;; list of yards in block adjacent to any patch
  vegetation-volume ;; number of plant species on a patch
  block-veg ;; sum of vegetation volume of entire block
...
to setup-blocks
  ...
  ask patches ;; combine these two into one ask statement
  [
    set block-mates patches with
    [block-number = [block-number] of myself]
  ] ;; adjusted so block-mates also includes self

  ask patches
  [
    set block-veg sum [ vegetation-volume ] of block-mates
  ]

  ask patches
  [
    set adjacent-block ; goal is for moving window type movement from birds, must test
    neighbors with [block-number != [block-number ] of myself]
  ]
end
...
to create-initial-output-files

  ;; Produces output on initial plant species occurance; each species in each patch is listed separately
  ;; I can't remember why I thought this file would be useful
  let output-list (list ( list "P-xcor" "P-ycor" "patch ID" "block ID" "Species" "Number in yard" ))
  ask patches
  [
    let patch-species-data table:to-list species-table
    foreach patch-species-data [ the-row ->
    set output-list lput (list pxcor pycor (word pxcor "_" pycor) block-number (first the-row) (last the-row)) output-list ]
  ]
  csv:to-file (word "SpeciesOccurrenceStart-" behaviorspace-run-number ".csv") output-list

  ;; Output distribution of intital patch richness; one row per patch
  ;; this is to allow comparison with empirical data
  let output-richness-list (list (list "pxcor" "pycor" "patch ID" "block ID" "patch richness"))
  ask patches
  [ set output-richness-list lput (list pxcor pycor (word pxcor "_" pycor) block-number vegetation-volume) output-richness-list ]
  csv:to-file (word "PatchRichnessStart-" behaviorspace-run-number ".csv") output-richness-list

  ;; outputing number of yards that have each plant species to start
  ;; summarizes species occurrence over all patches; should highly correlate with SpeciesList input file
  ;; this is just to check that species are being assigned to yards the way they should be
  let tablex table:make
  ...
  [ the-species -> let num-patches-with-species count patches with
    ...
    table:put tablex the-species num-patches-with-species]
  set tablex table:to-list tablex
  csv:to-file (word "NumberOfPatchesWithSpeciesStart-" behaviorspace-run-number ".csv") tablex

end

to go
  calculate-happiness
  ;test-sprout-happiness
  count-neighbors-species
  change-unhappy-patches
  kill-birds
  reproduce
  hatchling-move
  explore
  update-outputs
  tick
  ifelse (all? patches [happy?]) [stop] [if ticks >= 100 [stop]]
end


to calculate-happiness
  ;; determines the range of richness that will allow each patch to be happy based on neighbors' richness
  ...
  ;; in this version, patches want to have a similar number of species to their neighbors
  [ ask patches
  [
    ;; calculates mean richness in the neighboring patches
    let summed-richness sum [vegetation-volume] of neighbor-set
    set avg-richness round (summed-richness / neighbor-count)

    ;; calculates the range of values within which a patch can be happy
    let happy-min (avg-richness - yard-difference) ;; min number of species needed to be happy
    let happy-max (avg-richness + yard-difference) ;; max number of species to be happy

    ;; patch is happy if its species richness is similar to the rounded average richness of its neighbors
    set happy? (vegetation-volume <= happy-max AND vegetation-volume >= happy-min)
  ]]
  ...
  [
    ...
    let happy-min (avg-richness - yard-difference)

    ;; patch is happy if its species richness is similar to or greater than the rounded average richness of its neighbors
    set happy? (vegetation-volume >= happy-min) ;; patches are happy unless they have fewer species than their neighbors
   ]]

end

to test-sprout-happiness
  ;; test procedure to visualize if patches are happy
  ;; if patches are happy they sprout a smiley face
;;commented out b/c killing birds
  ;ask patches
  ;[if happy?
  ;[sprout 1 [set shape "face happy" set color yellow set size 0.25]]
  ;if not happy?
  ;[ask turtles-here [die]]
;]
end

to count-neighbors-species
  ;; this procedure determines which species to add or subtract from unhappy patches in the change-unhappy-patches procedure below

  ;; create neighbor-species-table that counts the number of neighbors with each species
  ask patches
  ...
  ;; this turns neighbor-species-table into a list with the most common species at the top
  ask patches
  [
...
to change-unhappy-patches
  ...
  [
   if (vegetation-volume < (avg-richness - yard-difference))
   [
   ;; create a temporary copy of sorted-list and remove species from its start until you reach one that current patch does not have
   ;; I struggled with this code and it probably could use a little more testing to make sure it's doing what it's supposed to do
   ...
   [ table:put species-table (first (first temp-sorted-list)) (1 + random 11) ] ;; adds the most common species to patch species-table, with a random abundance
    ]
...
to hatchling-move ;;maybe make this the same for settled birds too combined
  ask turtles with [ settled? = 0 ] ; from hatched turtles, decide if settled and move to the patch with block veg > 5
  [
    if [ block-veg ] of patch-here > 10 [ set settled? 1 set color red move-to patch-here ]
  ]
end
...
to explore ;;i want birds to look at adjacent blocks to the one they are settled on, see if block-veg is good enough, if not, die
  ...
  [
    if any? adjacent-block with [ block-veg > [block-veg] of self] ;;3 options, adjacent block, evaluate current space, or die
    [ move-to one-of adjacent-block ]
  ]
end

;; update colors and other display attributes and write output files here
to update-outputs
  ask patches
  [set vegetation-volume table:length species-table]  ;; updates vegetation-volume

  ;; useful visualizations but label doesn't work well if there are too many patches
  ;; turn off color while running behavior species
  ;  ask patches
  ;  [ifelse happy? [ set plabel word "happy " vegetation-volume ] [set plabel word "unhappy " vegetation-volume ]

  ask patches
  [set plabel vegetation-volume
   set pcolor scale-color green vegetation-volume (max [vegetation-volume] of patches) 0 ]

  ;; Produces updated output on plant species occurance; each species in each patch is listed separately
  let output-list (list ( list "P-xcor" "P-ycor" "patch ID" "Block ID" "Species" "Number in yard" ))
  ask patches
  [
    let patch-species-data table:to-list species-table
  ...
  csv:to-file (word "SpeciesOccurrenceEnd-" behaviorspace-run-number ".csv") output-list

  ;; output vegetation-volume for each patch
  let output-richness-list (list (list "pxcor" "pycor" "patch ID" "block ID" "patch richness"))
  ask patches
  [
    set output-richness-list lput (list pxcor pycor (word pxcor "_" pycor) block-number vegetation-volume) output-richness-list
  ...
  ;; outputing number of yards that have each plant species to end; can be used to produce a species rank-abundance curve
  let tablex table:make
  foreach plant-species-list
  [ the-species -> let num-patches-with-species count patches with
    [ table:has-key? species-table the-species ]
    table:put tablex the-species num-patches-with-species]
    set tablex table:to-list tablex
    csv:to-file (word "NumberOfPatchesWithSpeciesEnd-" behaviorspace-run-number ".csv") tablex

  ;; calculate block-level plant richness and abundance
  ;; I think this code doesn't work

  ; let block-list-table table:make (list (list remove-duplicates [block-number] of patches))
  ;  ;; show block-list
  ;  foreach block-list [block -> table:put block-species-table (block)]
end
@#$#@#$#@
GRAPHICS-WINDOW
420
