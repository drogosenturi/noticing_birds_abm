...
to setup-birds
  ...
  ask patches [ set bird-density count turtles-here ]
end

to setup-output
  ; create initial output files with the following headers
  if (file-exists? "bird-veg-output.csv")
  [
    carefully
    [file-delete "bird-veg-output.csv"]
    [print error-message]
  ]
  file-open "bird-veg-output.csv"
  file-type "patchid,"
  file-type "tick,"
  file-type "veg-volume,"
  file-type "bird-density,"
  file-print "max-bird-density"
  file-close
end
...
to birds-explore
  ...
  [
    ...
    ask suitable-habitat [ set pcolor blue ] ;; test to see suitable habitat
...
ask patches
  [
    ...
    file-type (word vegetation-volume ",")
    file-type (word bird-density ",")
    file-print (word max-bird-density ",")
  ]
  file-close
