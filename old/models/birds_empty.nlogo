...
to go
  set-density ;; try to relate this to habitat amount
  kill-birds
  reproduce
  hatchling-move
  explore
...
to kill-birds
  ...
  [
    die
    ;let suitable-habitat patches in-radius 2 with [ block-veg > 5 ] ;;probably get rid of this
...
to hatchling-move ;;maybe make this the same for settled birds too combined
  ...
  [
    if [ block-veg ] of patch-here > 10 and [ bird-density ] of patch-here < 3
    [
      set settled? 1
      set color red
      move-to patch-here
...
to explore ;;i want birds to look at adjacent blocks to the one they are settled on, see if block-veg is good enough, if not, die
  ...
  [
    if any? adjacent-block with [ block-veg > [ block-veg ] of self and bird-density < 3 ] ;;3 options, adjacent block, evaluate current space, or die
