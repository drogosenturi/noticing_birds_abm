...
;; neighbor-bird-estimate is removed in this version of the model
;; bird-estimates are a function of vegetation-volume, rather than a threshold
...
to update-estimates
  ...
  ask patches with [vegetation-volume <= ((mean [vegetation-volume] of patches) * .50)]
  [
    set yard-bird-estimate round (bird-density * random-float 2)
  ]

  ;; estimates of own yard are higher, 80-250% accurate
  ask patches with [vegetation-volume > ((mean [vegetation-volume] of patches) * .50)] ;;try to make function of mean-veg-volume
  [
    set yard-bird-estimate round (bird-density * ((random-float 2.5) + 0.50))
  ]
end
...
to change-bird-love ;; adjust this procedure to increase bird love based on birds observed in yard compared to neighbors' yards
  ...
  ask patches with [bird-love < 2]
  [
    ;if random-float 1 < (yard-bird-estimate * .1) [ set bird-love bird-love + 1 set change-count change-count + 1 ]
    if yard-bird-estimate > bird-density [ set bird-love bird-love + 1 set change-count change-count + 1 ]
  ]

  ask patches with [bird-love > 0] ;; figure out even chance to reduce bird-love as above
  [
    ;if random-float 1 > (yard-bird-estimate * .0001) [ set bird-love bird-love - 1 set change-count change-count + 1 ]
    if yard-bird-estimate < bird-density [ set bird-love bird-love - 1 set change-count change-count - 1 ]
  ]
...
to add-remove-vegetation ;; add vegetation if you love birds and you don't have enough in your yard
  ...
  [
    if random-float 1 > .90 [set vegetation-volume vegetation-volume + 1 ]
  ]
