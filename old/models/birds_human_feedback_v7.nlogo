...
to assign-bird-love
  loop
  [
    set bird-love round random-normal 5.0 2.12
...
to update-estimates
  ...
  ask patches
  [
    set yard-bird-estimate round (bird-density * (vegetation-volume * ((random-float 1) / 4)))
    print yard-bird-estimate * .1
  ]
end
...
to change-bird-love ;; adjust this procedure to increase bird love based on birds observed in yard
  ...
  ask patches with [bird-love < 10]
  [
    if random-float mean [bird-density] of patches * 0.1 < (yard-bird-estimate * .1) [ set bird-love bird-love + 1 set change-count change-count + 1 ]
  ]
