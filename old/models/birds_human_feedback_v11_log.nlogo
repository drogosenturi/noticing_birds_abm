;; ver 11: made edits with Emily, plan to finalize change-bird-love
...
to assign-bird-love
  ...
  ;; BIRD LOVE 0 OR 1 IDEA
;  let love-list [ 0 1 ]
;  set bird-love one-of love-list
end
...
to update-bird-estimates
  ...
  [
    ...
    set yard-bird-estimate round ((bird-density + (mean [bird-density] of neighbors)) * (vegetation-volume * ((random-float 1) / estimate-factor)))
    ;; estimate-factor adjusted by slider
...
to change-bird-love
  ;; QUARTILE IDEA
  let est-list (sort [yard-bird-estimate] of patches) ;; creates temp list of all yard-bird estimate values, currently printing all values as one item, fix to be individual items ;; fixed by excluding list prim
  print est-list
  let quartile round (count patches * 0.25)
  print word "quartile postion:" quartile
  ;; find the value in list that is quartile value from bottom and top and set bottom and top quart
  let bottom-quart item (quartile - 1) est-list ;; quartile - 1 because list starts from position 0
  print word "bottom quart:" bottom-quart
  let top-quart item (quartile - 1) reverse est-list
  print word "top-quart:" top-quart
  let patch-list sort-on [yard-bird-estimate] patches
  print word "patch-list:" patch-list
  let top-patches sublist patch-list 0 quartile
  print word "top-patches:" top-patches
  let bottom-patches sublist reverse patch-list 0 quartile
 print word "bottom-patches:" bottom-patches

;  ask patch-set top-patches
;  [
;    if random-float 1 <= change-chance
;    [
;      if yard-bird-estimate >= top-quart
;      [
;        if bird-love < 10 [set bird-love bird-love + 1 set increase-count increase-count + 1]
;      ]
;      if yard-bird-estimate <= bottom-quart
...
;    if random-float 1 <= change-chance
;    [
;      set bird-love 1 set increase-count increase-count + 1
;    ]
;  ]
;  ask patch-set bottom-patches
;  [
;    if random-float 1 <= change-chance
;      [ set bird-love 0 set decrease-count decrease-count + 1 ]
;  ]
  ...
  [
    if random-float 1 <= change-chance
    [
      set bird-love 2 + 1.39 * yard-bird-estimate + -0.97 * yard-bird-estimate + 0.42 * yard-bird-estimate + -.096 * yard-bird-estimate + 0.008 * yard-bird-estimate
    ]
  ]
...
;  [
;    ;print prev-bird-est
;    ;; change-chance slider on interface determines % chance to change bird-love
;    if random-float 1 <= change-chance
;    [
;      let midpoint mean [bird-density] of patches
;      let log-est (1 / (1 + (exp ((0 - log-slope) * (yard-bird-estimate - midpoint)))))
;      ifelse ((random-float 0.5) + 0.25) <= log-est
;      [ if bird-love < 10 [set bird-love bird-love + 1 set increase-count increase-count + 1] ] ;; compare how  many time increase vs decrease
;      [ if bird-love > 0 [set bird-love bird-love - 1 set decrease-count decrease-count + 1] ]
;    ]
;  ]

  ;; have a chance to downgrade your bird-love value due to random occurence if birds at carrying capacity in your yard, KEEP OR REMOVE?
;  ask patches
;  [
...
to change-vegetation ;; add vegetation based on bird-love, with 5% max chance to add or remove
  ...
  [
    ...
    let increase-chance bird-love * 0.05 ;; CHANGED FOR BIRD LOVE 0 OR 1
