...
[
 ...
 ;; mean-richness ;; the mean richness of yards, which is used as a parameter in random-exponential for assigning # of species to each year - now a slider on interface
...
to setup
  ...
  set plant-species-input csv:from-file "veg_structure_input_.csv"  ;; Read in the file
...
to setup-patches ;; procedure to assign plant species to each patch and identify neighbors
  ...
  ask patches
  [
    set patch-richness round random-gamma 2 2 ;; assigns patch richness randomly to yards ; change to mean-vegetation-volume
    ;if patch-richness > 12 [ set patch-richness round random-exponential mean-richness ]
    ;; 5.2 is mean of yard richness from empirical data - changed from random exponential b/c of error
    set plabel patch-richness ;; labels patch richness on interface, turned off for behavior space
  ]
  ...
  [ set species-table table:make
    foreach rnd:weighted-n-of-list patch-richness plant-species-input [ [a-species] -> last a-species]
...
to setup-blocks
  ...
  ask patches
  [ set block-mates other patches with [block-number = [block-number] of myself]] ;; I don't remember why I wrote this code but maybe it can be useful for calculating block-level richness
end
...
to calculate-happiness
  ...
  [
    ...
    set happy? (patch-richness >= happy-min) ;; patches are happy unless they have fewer species than their neighbors
   ]]
...
to change-unhappy-patches
  ...
  [
   if (patch-richness < (avg-richness - yard-difference))
   [
...
to update-outputs
  ask patches
  [set patch-richness table:length species-table]  ;; updates patch-richness
  ...
  ;  ask patches
  ;  [ifelse happy? [ set plabel word "happy " patch-richness ] [set plabel word "unhappy " patch-richness ]

  ask patches
  [set plabel patch-richness
   set pcolor scale-color green patch-richness (max [patch-richness] of patches) 0 ]
  ...
  ;; output patch-richness for each patch
  let output-richness-list (list (list "pxcor" "pycor" "patch ID" "block ID" "patch richness"))
  ask patches
  [
    set output-richness-list lput (list pxcor pycor (word pxcor "_" pycor) block-number patch-richness) output-richness-list
...
325
mean yard richness
mean  [patch-richness] of patches
1
1
11
...
standard-deviation [patch-richness] of patches
1
1
11
