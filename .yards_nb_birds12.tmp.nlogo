...
;;commented out b/c killing birds
  ask patches
  [if happy?
  [sprout-humans 1 [set shape "face happy" set color yellow set size 0.25]]
  if not happy?
  [ask humans-here [die]]
]
end
...
;to move-birds
 ; ask birds [
  ;  let roll random-float 1
   ; if roll < 0.5
    ;[ move-to one-of patches in-radius 60 with [ vegetation-volume > 3 ] ]
    ;[ die ]
  ;]
;end
...
to move-birds
  ask birds [
    let roll random-float 1
    ifelse one-of patches in-radius 60 with [ vegetation volume > 3 ]
    [ if roll < 0.5 [ move-to one-of patches in-radius 60 with [ vegetation-volume > 3 ] ]
    [ die ]
  ]
end
